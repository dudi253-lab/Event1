...
export async function createSession(values:{role:StaffRole;eventId?:string|null;credentialVersion:number;userId?:string}) {
  const token=randomToken(), db=serverClient();
  const {error}=await db.from('staff_sessions').insert({token_hash:sha256(token),role:values.role,event_id:values.eventId||null,user_id:values.userId||null,credential_version:values.credentialVersion,expires_at:new Date(Date.now()+8*3600_000).toISOString()});
...
export async function requireSession(eventId:string,roles:StaffRole[]) {
  ...
  if(!s || s.revoked_at || new Date(s.expires_at)<=new Date() || !roles.includes(s.role)) return null;
  const {data:event}=await db.from('events').select('company_id,credential_version').eq('id',eventId).single();
  if(!event || (s.role==='photo_moderator' && (s.event_id!==eventId || s.credential_version!==event.credential_version))) return null;
...
export async function requireAdmin(){const token=(await cookies()).get(SESSION_COOKIE)?.value;if(!token)return null;const db=serverClient();const {data:s}=await db.from('staff_sessions').select('id,role,user_id,expires_at,revoked_at').eq('token_hash',sha256(token)).maybeSingle();if(!s||s.role!=='company_admin'||!s.user_id||s.revoked_at||new Date(s.expires_at)<=new Date())return null;const {data:m}=await db.from('company_members').select('company_id,user_id').eq('user_id',s.user_id).eq('role','company_admin').limit(1).maybeSingle();return m?{...s,company_id:m.company_id}:null}
